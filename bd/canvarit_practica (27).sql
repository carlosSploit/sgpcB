...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 01-12-2022 a las 00:44:50
...
-- Procedimientos
--
CREATE PROCEDURE `analitic_ciclo_sincrono` (IN `id_curso` INT)   BEGIN
...
CREATE PROCEDURE `analitic_ciclo_sincrono_pru` (IN `id_curso` INT)   BEGIN
# LISTAR TODOS LOS CICLOS CON SUS ANALISIS RESPECTIVOS CON SUS INSCRITOS, SUS GANACIAS Y COMO LES FUE EN EL REGISTRO
SELECT dataanl.id_ciclo_curso, dataanl.fecha_init, dataanl.fecha_fin, dataanl.cantinscrip, dataanl.ganacia, dataanl.promedioasis,
CASE
    WHEN dataanl.promedioasis <= 36 THEN "MALO"
...
CREATE PROCEDURE `analitic_ciclo_sincrono_puntos` (IN `id_ciclocurso` INT)   BEGIN
...
CREATE PROCEDURE `compru_api_key` (IN `seccion_key` VARCHAR(100))  NO SQL BEGIN
# COMPRUEBA API KEY
SELECT sil.api_key FROM sing_log sil WHERE sil.seccion_key = seccion_key;
END$$

CREATE PROCEDURE `Compru_datuserlog` (IN `seccion_key` VARCHAR(100))  NO SQL BEGIN
...
CREATE PROCEDURE `compru_logusser` (IN `usser` VARCHAR(100), IN `pass` VARCHAR(15))  NO SQL BEGIN
...
CREATE PROCEDURE `Compru_rescupass` (IN `correo` VARCHAR(100))  NO SQL BEGIN
...
CREATE PROCEDURE `delect_admin` (IN `id_adm` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `delect_alumno` (IN `id_alum` INT)  NO SQL BEGIN 
# EXTRAER EL ID DE USUARIO DEL ADMIN 
...
CREATE PROCEDURE `delect_pointclas` (IN `id_pointclass` INT)   BEGIN
# ELIMINAR UN PUNTO
UPDATE `puntosclass` SET stade = 0 WHERE id_tipo_puntos = id_pointclass;
END$$

CREATE PROCEDURE `delect_profe` (IN `id_profe` INT)  NO SQL BEGIN
# EXTRAER EL ID DE USUARIO DEL ADMIN 
...
CREATE PROCEDURE `delect_sesion` (IN `id_sesion` INT)  NO SQL BEGIN
# ELIMINAR LA SESION DEL CURSO
UPDATE `sesiones` SET `estado`= 0 WHERE `sesiones`.`id_sesion` = id_sesion ;
END$$

CREATE PROCEDURE `delect_tareainscr` (IN `id_tareinsc` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `delect_tipocurso` (IN `id_tipocurso` INT)  NO SQL BEGIN
# ELIMINAR UN TIPO DE CURSO
UPDATE `tipo_curso` SET `estado` = '0' WHERE `tipo_curso`.`id_tipocurso` = id_tipocurso;
END$$

CREATE PROCEDURE `insert_admin` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `tipo_admin` VARCHAR(100), IN `photo` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `insert_alumno` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `name_tutor` VARCHAR(100), IN `edad` INT(3), IN `id_tutoralumno` INT)  NO SQL BEGIN 
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE PROCEDURE `insert_apertureasist` (IN `id_sesion` INT)   BEGIN
...
CREATE PROCEDURE `insert_ciclocurso` (IN `id_curso` INT, IN `id_profesor` INT, IN `fechaini` DATE, IN `fechafin` DATE, IN `diasdur` INT, IN `presio` DECIMAL)  NO SQL BEGIN
...
CREATE PROCEDURE `insert_contsession` (IN `id_sesion` INT, IN `id_tipoconte` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `insert_conttarea` (IN `id_sesion` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `insert_curso` (IN `idtipcur` INT, IN `name` VARCHAR(100), IN `descr` VARCHAR(500), IN `alcan` VARCHAR(500), IN `contvid` VARCHAR(2000), IN `photpo` VARCHAR(400))  NO SQL BEGIN
# INSERTAR CURSO
...
CREATE PROCEDURE `insert_inscrip` (IN `idalumn` INT, IN `idciclcurso` INT, IN `idtipoinscrip` INT, IN `urlvoucher` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `insert_inscripuntclas` (IN `id_inscri` INT, IN `id_tipopunt` INT, IN `id_sesion` INT)   BEGIN
...
CREATE PROCEDURE `insert_pointclas` (IN `name` VARCHAR(100), IN `valpoint` INT, IN `photo` VARCHAR(400), IN `id_prof` INT, IN `isacumulado` INT, IN `isdefault` INT)   BEGIN
...
CREATE PROCEDURE `insert_profe` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `estudios` VARCHAR(200))  NO SQL BEGIN 
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE PROCEDURE `insert_sesion` (IN `name` VARCHAR(100), IN `id_ciclocurso` INT)  NO SQL BEGIN
# INSERTAR SESION DEL CURSO
INSERT INTO `sesiones`(`id_ciclocurso`, `nombre`, `estado`) VALUES (id_ciclocurso, name, 1);
END$$

CREATE PROCEDURE `insert_singlog` (IN `id_usser` INT, IN `seccion_key` VARCHAR(100), IN `api_ley` VARCHAR(400))  NO SQL BEGIN
# INSERTAR UN SINGLOG
...
CREATE PROCEDURE `insert_tareainscr` (IN `id_inscr` INT, IN `id_conttar` INT, IN `urlconte` VARCHAR(400))  NO SQL BEGIN
# INGRESAR UNA TAREA
INSERT INTO `tarea_conte_inscr`(`id_inscrip`, `id_conte_tarea`, `urlconte`, `puntos`, `estado`) VALUES (id_inscr, id_conttar, urlconte, 0, 1);
END$$

CREATE PROCEDURE `insert_tipocurso` (IN `name` VARCHAR(100))  NO SQL BEGIN
# INSERT TIPO DE CURSO
INSERT INTO `tipo_curso`(`name`, `estado`) VALUES (name,1);
END$$

CREATE PROCEDURE `insert_tutAlumn` (IN `nombre_tutAlum` VARCHAR(100), IN `correo` VARCHAR(200), IN `telf` VARCHAR(9), IN `photo` VARCHAR(500), IN `pass` VARCHAR(15))   BEGIN
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE PROCEDURE `list_admin` (IN `name` VARCHAR(100))  NO SQL BEGIN
...
CREATE PROCEDURE `list_alumno` (IN `name` VARCHAR(100))  NO SQL BEGIN
# LISTAR TODOS LOS USUARIOS ACTIVOS
...
CREATE PROCEDURE `list_apertureasis` (IN `id_ciclocurso` INT)   BEGIN
...
CREATE PROCEDURE `list_inscripasisten` (IN `id_apertuasis` INT)   BEGIN
...
CREATE PROCEDURE `list_inscripuntclas` (IN `id_inscri` INT, IN `id_sesion` INT, IN `id_ciclo` INT)   BEGIN
...
CREATE PROCEDURE `list_inscri_alumn` (IN `id_ciclocur` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `list_inscri_curso` (IN `id_alumn` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `list_pointclas` (IN `id_prof` INT)   BEGIN
...
CREATE PROCEDURE `list_preinscrip` ()  NO SQL SELECT ins.id_inscrip, ins.fechainscri, usa.photo ,al.id_alumno, usa.nombre, cc.id_ciclo_curso, c.id_curso, c.nombre as namecurso, cc.presio_inscri, ins.url_voucher
FROM inscripcciones ins
INNER JOIN ciclo_curso cc ON cc.id_ciclo_curso = ins.id_ciclo_curso
INNER JOIN curso c ON c.id_curso = cc.id_curso
INNER JOIN alumno al ON al.id_alumno = ins.id_alumno
...
CREATE PROCEDURE `list_profe` (IN `name` VARCHAR(100))  NO SQL BEGIN
...
CREATE PROCEDURE `list_sesion` (IN `id_ciclocurso` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `list_tareainscr` (IN `id_conttar` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `list_tareainscr_ranking` (IN `id_ciclocurs` VARCHAR(100))   BEGIN
...
CREATE PROCEDURE `read_admin` (IN `id_admin` INT)  NO SQL BEGIN
# LEER EL ADMIN
SELECT ad.id_admin ,us.nombre, us.correo, us.telf, us.pass, us.photo,  ad.tipo_trabajador
FROM admin ad
INNER JOIN usuario us ON us.id_usuario = ad.id_usuario 
...
CREATE PROCEDURE `read_alumno` (IN `id_alumno` INT)  NO SQL BEGIN
# LEER EL ALUMNO
SELECT al.id_alumno ,us.nombre, us.correo, us.telf, us.pass, us.photo, al.edad, al.name_tutor, al.id_tutor_alumno
FROM alumno al 
INNER JOIN usuario us ON us.id_usuario = al.id_usuario WHERE us.estado = 1
...
CREATE PROCEDURE `read_asisten_info` (IN `id_asisten_inscr` INT)   BEGIN
...
CREATE PROCEDURE `read_ciclocurso` (IN `id_curso` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `read_conttarea` (IN `id_conttar` INT)   BEGIN
# LEER LA INFORMACION DEL CONTENIDO DE TAREA
SELECT * FROM conte_tarea ct WHERE ct.id_conte_tarea = id_conttar ;
END$$

CREATE PROCEDURE `read_idinscripconttar` (IN `id_conttar` INT, IN `id_alum` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `read_profe` (IN `id_profe` INT)  NO SQL BEGIN
# LISTAR LOS PROFESORES
SELECT prof.id_profesor ,us.nombre, us.correo, us.telf, us.pass, us.photo,  prof.estudios
FROM profesor prof
INNER JOIN usuario us ON us.id_usuario = prof.id_usuario 
...
CREATE PROCEDURE `update_admin` (IN `id_admin` INT, IN `name` VARCHAR(100), IN `correo` VARCHAR(100), IN `telf` INT(9), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `tipo_trab` VARCHAR(100))  NO SQL BEGIN
...
CREATE PROCEDURE `update_alumno` (IN `id_alumno` INT, IN `name` VARCHAR(100), IN `correo` VARCHAR(100), IN `telf` INT(9), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `edad` INT(3), IN `name_tutro` VARCHAR(100), IN `id_tutoralumno` INT)  NO SQL BEGIN
#ACTUALIZAR LA INFO DEL PROFESOR
...
CREATE PROCEDURE `update_ciclocourseStade` ()  NO SQL BEGIN
...
CREATE PROCEDURE `update_ciclocurso` (IN `idciclcur` INT, IN `idcurso` INT, IN `idprofe` INT, IN `fechini` DATE, IN `fechfin` DATE, IN `diasduc` INT, IN `presio` DECIMAL)  NO SQL BEGIN
...
CREATE PROCEDURE `update_contsesion` (IN `id_conteses` INT, IN `id_tipoconte` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `update_conttarea` (IN `id_contacti` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `update_curso` (IN `id_curso` INT, IN `id_tipcur` INT, IN `name` VARCHAR(100), IN `descr` VARCHAR(500), IN `alcan` VARCHAR(500), IN `contvid` VARCHAR(2000), IN `photpo` VARCHAR(400))  NO SQL BEGIN
...
CREATE PROCEDURE `update_pointclas` (IN `name` VARCHAR(100), IN `valpoint` INT, IN `photo` VARCHAR(400), IN `id_pointclass` INT, IN `isacumulado` INT)   BEGIN
...
CREATE PROCEDURE `update_profesor` (IN `id_profe` INT, IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `estudios` VARCHAR(200))  NO SQL BEGIN
#ACTUALIZAR LA INFO DEL PROFESOR
...
CREATE PROCEDURE `update_sesion` (IN `id_sesion` INT, IN `name` VARCHAR(100))  NO SQL BEGIN
# ACTUALIZAR LA SESION DEL CURSO
UPDATE `sesiones` SET `nombre`= name WHERE `sesiones`.`id_sesion` = id_sesion;
END$$

CREATE PROCEDURE `update_stadeinscrip` (IN `idinscrp` INT, IN `estade` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `update_stadoasisten` (IN `id_asisins` INT, IN `estade` INT)   BEGIN
...
CREATE PROCEDURE `update_tareainsc` (IN `id_tarins` INT, IN `urlcont` VARCHAR(400))  NO SQL BEGIN
UPDATE `tarea_conte_inscr`
SET `urlconte`= urlcont
WHERE `tarea_conte_inscr`.`id_tarea_inscri` = id_tarins ;
END$$

CREATE PROCEDURE `update_tareainsc_ponint` (IN `id_tarinsc` INT, IN `point` INT)  NO SQL BEGIN
...
CREATE PROCEDURE `update_tipocurso` (IN `id_tipocurso` INT, IN `name` VARCHAR(100))  NO SQL BEGIN
# ACTUALIZAR EL TIPO DE TRABAJO
UPDATE `tipo_curso` SET `name` = name WHERE `tipo_curso`.`id_tipocurso` = id_tipocurso ;
END$$

CREATE PROCEDURE `valider_couse_ciclo` (IN `id_curso` INT)  NO SQL BEGIN
# ACTUALIZAR ESTADO DE LOS CURSOS
# -- Actualizar si el curso ya empeso
UPDATE `ciclo_curso`
SET `estado` = 2
...
CREATE PROCEDURE `valider_inscpointclas` (IN `id_sesion` INT, IN `id_inscrip` INT, IN `id_tipopunt` INT)   BEGIN
...
CREATE PROCEDURE `valider_inscricurse` (IN `idalumno` INT, IN `idciclocur` INT)  NO SQL BEGIN
...
-- Funciones
--
CREATE FUNCTION `promedio_asisten_ciclo` (`id_ciclo` INT) RETURNS DECIMAL(10,0) UNSIGNED NO SQL BEGIN
...
--
ALTER TABLE `ciclo_curso`
  MODIFY `id_ciclo_curso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=34;
...
--
ALTER TABLE `inscripcciones`
  MODIFY `id_inscrip` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=52;
...
--
ALTER TABLE `puntosclass`
  MODIFY `id_tipo_puntos` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=78;
...
--
ALTER TABLE `sing_log`
  MODIFY `id_sing_log` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=293;
