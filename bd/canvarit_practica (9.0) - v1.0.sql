...
-- Tiempo de generación: 19-09-2022 a las 20:26:37
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_alumno` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `name_tutor` VARCHAR(100), IN `edad` INT(3))  NO SQL
BEGIN 
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_apertureasist` (IN `id_ciclo` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_alumno` (IN `id_alumno` INT, IN `name` VARCHAR(100), IN `correo` VARCHAR(100), IN `telf` INT(9), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `edad` INT(3), IN `name_tutro` VARCHAR(100))  NO SQL
BEGIN
#ACTUALIZAR LA INFO DEL PROFESOR
