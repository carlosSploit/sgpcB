...
-- Tiempo de generación: 01-12-2022 a las 21:13:25
...
-- Procedimientos
--
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `analitic_ciclo_sincrono` (IN `id_curso` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `analitic_ciclo_sincrono_pru` (IN `id_curso` INT)  BEGIN
# LISTAR TODOS LOS CICLOS CON SUS ANALISIS RESPECTIVOS CON SUS INSCRITOS, SUS GANACIAS Y COMO LES FUE EN EL REGISTRO
SELECT dataanl.id_ciclo_curso, dataanl.fecha_init, dataanl.fecha_fin, dataanl.cantinscrip, dataanl.ganacia, dataanl.promedioasis,
CASE
    WHEN dataanl.promedioasis <= 36 THEN "MALO"
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `analitic_ciclo_sincrono_puntos` (IN `id_ciclocurso` INT)  BEGIN
# ANALIZA LOS PUNTOS DADOS EN UN ALUMNO EN UNA SESION
SELECT insdat.id_ciclo_curso, insdat.id_inscrip, insdat.nombre, insdat.photo, (point_total_inscrito(insdat.id_inscrip)) as sumatotal 
FROM (SELECT cc.id_ciclo_curso, ins.id_inscrip , us.nombre, us.photo 
      ...
      AND cc.id_ciclo_curso = 34 
      AND ins.estado <> 0) as insdat;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `compru_api_key` (IN `seccion_key` VARCHAR(100))  NO SQL
BEGIN
# COMPRUEBA API KEY
SELECT sil.api_key FROM sing_log sil WHERE sil.seccion_key = seccion_key;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `Compru_datuserlog` (IN `seccion_key` VARCHAR(100))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `compru_logusser` (IN `usser` VARCHAR(100), IN `pass` VARCHAR(15))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `Compru_rescupass` (IN `correo` VARCHAR(100))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_admin` (IN `id_adm` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_alumno` (IN `id_alum` INT)  NO SQL
BEGIN 
# EXTRAER EL ID DE USUARIO DEL ADMIN 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_asistencia` (IN `id_session` INT)  BEGIN
UPDATE `asistencia` SET `asistencia`.`estade`= 0 WHERE `asistencia`.`id_sesion` = id_session ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_ciclocurso` (IN `id_ciclocurso` INT)  NO SQL
BEGIN
# ELIMINAR EL CICLO DE UN CURSO
UPDATE `ciclo_curso` SET `estado`= 0 WHERE `ciclo_curso`.`id_ciclo_curso` = id_ciclocurso;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_contsessi` (IN `id_conteses` INT)  NO SQL
BEGIN
# ELIMINAR EL CONTENIDO DE UNA SESSION
UPDATE `conte_sesion` SET `estado`= 0 WHERE `conte_sesion`.`id_conte_sesion`= id_conteses ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_conttarea` (IN `id_contacti` INT)  NO SQL
BEGIN
# ELIMINAR UN CONTENIDO DE TAREA
UPDATE `conte_tarea` SET `estado`= 0 WHERE `conte_tarea`.`id_conte_tarea` = id_contacti ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_curso` (IN `id_curso` INT)  NO SQL
BEGIN
# ELIMINAR EL CURSO
UPDATE `curso` SET `estado`= 0 WHERE `curso`.`id_curso` = id_curso ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_inscripuntclas` (IN `id_sesion` INT, IN `id_inscrip` INT, IN `id_tipopunt` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_pointclas` (IN `id_pointclass` INT)  BEGIN
# ELIMINAR UN PUNTO
UPDATE `puntosclass` SET stade = 0 WHERE id_tipo_puntos = id_pointclass;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_profe` (IN `id_profe` INT)  NO SQL
BEGIN
# EXTRAER EL ID DE USUARIO DEL ADMIN 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_sesion` (IN `id_sesion` INT)  NO SQL
BEGIN
# ELIMINAR LA SESION DEL CURSO
UPDATE `sesiones` SET `estado`= 0 WHERE `sesiones`.`id_sesion` = id_sesion ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_tareainscr` (IN `id_tareinsc` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `delect_tipocurso` (IN `id_tipocurso` INT)  NO SQL
BEGIN
# ELIMINAR UN TIPO DE CURSO
UPDATE `tipo_curso` SET `estado` = '0' WHERE `tipo_curso`.`id_tipocurso` = id_tipocurso;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_admin` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `tipo_admin` VARCHAR(100), IN `photo` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_alumno` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `name_tutor` VARCHAR(100), IN `edad` INT(3), IN `id_tutoralumno` INT)  NO SQL
BEGIN 
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_apertureasist` (IN `id_sesion` INT)  BEGIN
# APERTURAR UNA ASISTENCIA
INSERT INTO `asistencia`(`fecha_asist`,`id_sesion`,`estade`) VALUES (now(), id_sesion, 1) ;
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_ciclocurso` (IN `id_curso` INT, IN `id_profesor` INT, IN `fechaini` DATE, IN `fechafin` DATE, IN `diasdur` INT, IN `presio` DECIMAL)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_contsession` (IN `id_sesion` INT, IN `id_tipoconte` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_conttarea` (IN `id_sesion` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_curso` (IN `idtipcur` INT, IN `name` VARCHAR(100), IN `descr` VARCHAR(500), IN `alcan` VARCHAR(500), IN `contvid` VARCHAR(2000), IN `photpo` VARCHAR(400))  NO SQL
BEGIN
# INSERTAR CURSO
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_inscrip` (IN `idalumn` INT, IN `idciclcurso` INT, IN `idtipoinscrip` INT, IN `urlvoucher` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_inscripuntclas` (IN `id_inscri` INT, IN `id_tipopunt` INT, IN `id_sesion` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_pointclas` (IN `name` VARCHAR(100), IN `valpoint` INT, IN `photo` VARCHAR(400), IN `id_prof` INT, IN `isacumulado` INT, IN `isdefault` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_profe` (IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `estudios` VARCHAR(200))  NO SQL
BEGIN 
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_sesion` (IN `name` VARCHAR(100), IN `id_ciclocurso` INT)  NO SQL
BEGIN
# INSERTAR SESION DEL CURSO
INSERT INTO `sesiones`(`id_ciclocurso`, `nombre`, `estado`) VALUES (id_ciclocurso, name, 1);
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_singlog` (IN `id_usser` INT, IN `seccion_key` VARCHAR(100), IN `api_ley` VARCHAR(400))  NO SQL
BEGIN
# INSERTAR UN SINGLOG
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_tareainscr` (IN `id_inscr` INT, IN `id_conttar` INT, IN `urlconte` VARCHAR(400))  NO SQL
BEGIN
# INGRESAR UNA TAREA
INSERT INTO `tarea_conte_inscr`(`id_inscrip`, `id_conte_tarea`, `urlconte`, `puntos`, `estado`) VALUES (id_inscr, id_conttar, urlconte, 0, 1);
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_tipocurso` (IN `name` VARCHAR(100))  NO SQL
BEGIN
# INSERT TIPO DE CURSO
INSERT INTO `tipo_curso`(`name`, `estado`) VALUES (name,1);
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `insert_tutAlumn` (IN `nombre_tutAlum` VARCHAR(100), IN `correo` VARCHAR(200), IN `telf` VARCHAR(9), IN `photo` VARCHAR(500), IN `pass` VARCHAR(15))  BEGIN
# INSERTAR LOS DATOS AL USUARIO 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_admin` (IN `name` VARCHAR(100))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_alumno` (IN `name` VARCHAR(100))  NO SQL
BEGIN
# LISTAR TODOS LOS USUARIOS ACTIVOS
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_apertureasis` (IN `id_ciclocurso` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_ciclocursoCurso` (IN `id_curso` INT)  NO SQL
BEGIN
# LISTAR LOS CICLOS DE UN CURSO EN ESPESIFICO
SELECT * FROM ciclo_curso cc WHERE cc.estado <> 0 AND cc.id_curso = id_curso ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_coment_canv` ()  NO SQL
BEGIN
SELECT * FROM correoslp WHERE 1;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_contsesi` (IN `id_sesion` INT)  NO SQL
BEGIN
# LISTAR LOS CONTENIDOS POR SESSION
SELECT * FROM conte_sesion cs WHERE cs.id_sesion = id_sesion AND cs.estado <> 0;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_conttarea` (IN `id_sesion` INT)  NO SQL
BEGIN
SELECT * FROM conte_tarea ct WHERE ct.estado <> 0 AND ct.id_sesion = id_sesion ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_curso` (IN `name` VARCHAR(100))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_inscripasisten` (IN `id_apertuasis` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_inscripuntclas` (IN `id_inscri` INT, IN `id_sesion` INT, IN `id_ciclo` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_inscri_alumn` (IN `id_ciclocur` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_inscri_curso` (IN `id_alumn` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_pointclas` (IN `id_prof` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_preinscrip` ()  NO SQL
SELECT ins.id_inscrip, ins.fechainscri, usa.photo ,al.id_alumno, usa.nombre, cc.id_ciclo_curso, c.id_curso, c.nombre as namecurso, cc.presio_inscri, ins.url_voucher
FROM inscripcciones ins
INNER JOIN ciclo_curso cc ON cc.id_ciclo_curso = ins.id_ciclo_curso
INNER JOIN curso c ON c.id_curso = cc.id_curso
INNER JOIN alumno al ON al.id_alumno = ins.id_alumno
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_profe` (IN `name` VARCHAR(100))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_sesion` (IN `id_ciclocurso` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_tareainscr` (IN `id_conttar` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_tareainscr_ranking` (IN `id_ciclocurs` VARCHAR(100))  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_tipoconte` ()  NO SQL
BEGIN
SELECT * FROM tipo_conte tc WHERE tc.estado = 1 ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_tipocurso` ()  NO SQL
BEGIN
# LISTAR LOS TIPOS DE TRABAJADORES
SELECT tc.id_tipocurso , tc.name FROM tipo_curso tc WHERE tc.estado = 1;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `list_tutAlumn` ()  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_admin` (IN `id_admin` INT)  NO SQL
BEGIN
# LEER EL ADMIN
SELECT ad.id_admin ,us.nombre, us.correo, us.telf, us.pass, us.photo,  ad.tipo_trabajador
FROM admin ad
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_alumno` (IN `id_alumno` INT)  NO SQL
BEGIN
# LEER EL ALUMNO
SELECT al.id_alumno ,us.nombre, us.correo, us.telf, us.pass, us.photo, al.edad, al.name_tutor, al.id_tutor_alumno
FROM alumno al 
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_asisten_info` (IN `id_asisten_inscr` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_ciclocurso` (IN `id_curso` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_conttarea` (IN `id_conttar` INT)  BEGIN
# LEER LA INFORMACION DEL CONTENIDO DE TAREA
SELECT * FROM conte_tarea ct WHERE ct.id_conte_tarea = id_conttar ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_idinscripconttar` (IN `id_conttar` INT, IN `id_alum` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `read_profe` (IN `id_profe` INT)  NO SQL
BEGIN
# LISTAR LOS PROFESORES
SELECT prof.id_profesor ,us.nombre, us.correo, us.telf, us.pass, us.photo,  prof.estudios
FROM profesor prof
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_admin` (IN `id_admin` INT, IN `name` VARCHAR(100), IN `correo` VARCHAR(100), IN `telf` INT(9), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `tipo_trab` VARCHAR(100))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_alumno` (IN `id_alumno` INT, IN `name` VARCHAR(100), IN `correo` VARCHAR(100), IN `telf` INT(9), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `edad` INT(3), IN `name_tutro` VARCHAR(100), IN `id_tutoralumno` INT)  NO SQL
BEGIN
#ACTUALIZAR LA INFO DEL PROFESOR
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_ciclocourseStade` ()  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_ciclocurso` (IN `idciclcur` INT, IN `idcurso` INT, IN `idprofe` INT, IN `fechini` DATE, IN `fechfin` DATE, IN `diasduc` INT, IN `presio` DECIMAL)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_contsesion` (IN `id_conteses` INT, IN `id_tipoconte` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_conttarea` (IN `id_contacti` INT, IN `name` VARCHAR(100), IN `descripcc` VARCHAR(300), IN `url_cont` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_curso` (IN `id_curso` INT, IN `id_tipcur` INT, IN `name` VARCHAR(100), IN `descr` VARCHAR(500), IN `alcan` VARCHAR(500), IN `contvid` VARCHAR(2000), IN `photpo` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_pointclas` (IN `name` VARCHAR(100), IN `valpoint` INT, IN `photo` VARCHAR(400), IN `id_pointclass` INT, IN `isacumulado` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_profesor` (IN `id_profe` INT, IN `name` VARCHAR(100), IN `telf` INT(9), IN `correo` VARCHAR(100), IN `pass` VARCHAR(15), IN `photo` VARCHAR(400), IN `estudios` VARCHAR(200))  NO SQL
BEGIN
#ACTUALIZAR LA INFO DEL PROFESOR
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_sesion` (IN `id_sesion` INT, IN `name` VARCHAR(100))  NO SQL
BEGIN
# ACTUALIZAR LA SESION DEL CURSO
UPDATE `sesiones` SET `nombre`= name WHERE `sesiones`.`id_sesion` = id_sesion;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_stadeinscrip` (IN `idinscrp` INT, IN `estade` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_stadoasisten` (IN `id_asisins` INT, IN `estade` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_tareainsc` (IN `id_tarins` INT, IN `urlcont` VARCHAR(400))  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_tareainsc_ponint` (IN `id_tarinsc` INT, IN `point` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `update_tipocurso` (IN `id_tipocurso` INT, IN `name` VARCHAR(100))  NO SQL
BEGIN
# ACTUALIZAR EL TIPO DE TRABAJO
UPDATE `tipo_curso` SET `name` = name WHERE `tipo_curso`.`id_tipocurso` = id_tipocurso ;
END$$

CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `valider_couse_ciclo` (IN `id_curso` INT)  NO SQL
BEGIN
# ACTUALIZAR ESTADO DE LOS CURSOS
# -- Actualizar si el curso ya empeso
UPDATE `ciclo_curso`
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `valider_inscpointclas` (IN `id_sesion` INT, IN `id_inscrip` INT, IN `id_tipopunt` INT)  BEGIN
...
CREATE DEFINER=`canvarit`@`localhost` PROCEDURE `valider_inscricurse` (IN `idalumno` INT, IN `idciclocur` INT)  NO SQL
BEGIN
...
-- Funciones
--
CREATE DEFINER=`canvarit`@`localhost` FUNCTION `point_total_inscrito` (`id_inscrip` INT) RETURNS DECIMAL(10,0) UNSIGNED NO SQL
BEGIN
# RETORNAR LOS LOS PUNTOS TOTALES DE UN INSCRITO
SET @result = (SELECT SUM(pc.value_point) as sumatotal 
FROM inscrip_point_sesion ips 
INNER JOIN puntosclass pc ON pc.id_tipo_puntos = ips.id_tipo_punto 
...
AND ins.id_inscrip = id_inscrip
AND ips.stade <> 0) ;
return @result;
END$$

CREATE DEFINER=`canvarit`@`localhost` FUNCTION `promedio_asisten_ciclo` (`id_ciclo` INT) RETURNS DECIMAL(10,0) UNSIGNED NO SQL
BEGIN
...
(19, 16, 'desconocido', 969280255, 49, 0),
(20, 16, 'desconocido', 962526447, 52, 0),
(21, 16, 'desconocido', 969280255, 55, 0);
...
(34, 18, 17, '2022-12-21', '2022-12-31', '0', 9, 1);
...
(18, 15, 'Generic 1', 'Curso Basico version 1', 'Curso Basico Version 2', '<div> No se encuentra ningun video </div>', 'http://res.cloudinary.com/canvarith/image/upload/v1669944957/Images/tsubdhtopzfzbvy4jchz.jpg', 1);
...
(52, 19, 34, 1, '', 1, '2022-12-01 20:25:39');
...
(78, 'Asistencia', 1, '1', 17, 1, 0, 1),
(79, 'Participacion', 1, '4', 17, 1, 1, 1),
(80, 'Resolvio Tarea', 1, '6', 17, 1, 1, 1);
...
(293, '039b5c4d-1843-483e-b577-cb90ab368772', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0MzI2NX0.30LYy-9UCiyju0Gy9OLcYDEXzk4tSBBRBaz0TLxeemE', '2022-12-01 19:07:45', 50),
(294, 'e55b50dd-3cbe-4d8e-bc35-f84d43c80e1c', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NDYwNn0.cCeaPMpECbHLBmoT32Itnp3U40HtNE8ZQjVE5STqCf4', '2022-12-01 19:30:06', 50),
(295, '76f7e0f7-94f8-4a06-b17f-6549a736f240', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NDY3OX0.4Gu1Pg5UoIA1Znt2EJTrVkyz0h01upKOzAmsK0GO-5Q', '2022-12-01 19:31:19', 50),
(296, '1dc4ac6e-b53e-431b-8e27-7026293e827f', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NDg2OH0.0V66oUle4swQIK8TOtlnKs4-KHwbjT0lcl_bpPGk4eM', '2022-12-01 19:34:28', 50),
(297, '0964a321-f3a1-469e-a3a5-f7c22e53ec41', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NTMxOX0.BDX6HoOASlmKL3e-5HzVk4OoL6QaA31KRYSiGNyG4C4', '2022-12-01 19:41:59', 50),
(298, '488643be-f5cb-45b3-8368-c5f9f4228437', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NTQzMX0.mYPKRqDvHZ8dmlNtThEFreRYkW83BdAxdttAfLzumvk', '2022-12-01 19:43:51', 51),
(299, 'b95ba582-2d4d-4e90-9a16-1b71dc884b37', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NzM3MH0.atgHzH8ee1YE685sS41zf2hqAQJ-V5aFOcjimPtULSw', '2022-12-01 20:16:10', 49),
(300, '62d6cbd0-4e83-4a5a-96b6-594f138ee6f5', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NzM5Nn0.fDHWKwDNYuDMr0dcyo5A2iEL1hdZxAt_Y274yNw5-5s', '2022-12-01 20:16:37', 49),
(301, 'f3b814ea-e35f-47b2-9455-1cb1d35f24a7', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NzQyNn0.10bYAddfNDsXUysNpxPgr20C2-FNKhBxhqarZ1x6in8', '2022-12-01 20:17:07', 49),
(302, '9b1d35d1-06e9-4d4f-a1cf-ebc3824f8a71', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0NzU2NH0.ALul5OpRrHsG_npMjIMks7YyIQLE_nc6aGYoz4AkdZ0', '2022-12-01 20:19:24', 49),
(303, '4852683c-22da-44f3-8a90-9903bbccf542', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0Nzc1Nn0.Wpjn70SEwdVeUUqguC28-6-B0B-J0pm7X7sQP1q6ZQw', '2022-12-01 20:22:36', 49),
(304, '54929a97-c680-465d-9c98-fa475337c9a9', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjo5NjkyODAyNTUsIm5vbWJyZSI6ImJyYWRjYXIiLCJlbWFpbCI6InVjdjIwMjFAZW1haWwuY29tIn0sImlhdCI6MTY2OTk0ODc1N30.jcEwPIMhW1gBrXnSgjHgwS-BPdFBo0Sm_muaYj8BGRw', '2022-12-01 20:39:17', 49);
...
--
ALTER TABLE `asistencia`
  MODIFY `id_asisten` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=22;

--
-- AUTO_INCREMENT de la tabla `asist_inscrip`
--
ALTER TABLE `asist_inscrip`
  MODIFY `id_asis_inscrip` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=48;

--
-- AUTO_INCREMENT de la tabla `ciclo_curso`
--
ALTER TABLE `ciclo_curso`
  MODIFY `id_ciclo_curso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=35;
...
--
ALTER TABLE `inscripcciones`
  MODIFY `id_inscrip` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=53;

--
-- AUTO_INCREMENT de la tabla `inscrip_point_sesion`
--
ALTER TABLE `inscrip_point_sesion`
  MODIFY `id_inscrip_p_sea` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=71;
...
--
ALTER TABLE `puntosclass`
  MODIFY `id_tipo_puntos` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=81;

--
-- AUTO_INCREMENT de la tabla `sesiones`
--
ALTER TABLE `sesiones`
  MODIFY `id_sesion` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=55;

--
-- AUTO_INCREMENT de la tabla `sing_log`
--
ALTER TABLE `sing_log`
  MODIFY `id_sing_log` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=305;
...
--
ALTER TABLE `tipo_curso`
  MODIFY `id_tipocurso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=89;
