...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 29-09-2022 a las 22:00:00
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `delect_inscripuntclas` (IN `id_inscrp_p_sea` INT)   BEGIN
# ELIMINAR LOS PUNTOS DE UN INSCRITO
UPDATE `inscrip_point_sesion` SET `stade` = 0 WHERE id_inscrip_p_sea = id_inscrp_p_sea ;
END$$
...
FROM inscrip_point_sesion ips
WHERE ips.id_inscrip = id_sesion
AND ips.id_sesion =  id_inscrip
...
--
ALTER TABLE `inscrip_point_sesion`
  MODIFY `id_inscrip_p_sea` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
ALTER TABLE `sing_log`
  MODIFY `id_sing_log` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=126;
