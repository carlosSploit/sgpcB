...
module.exports = class ngclasscontrol {
  async inser_classcontrol (req, res) {
    const result = await objclasscontroles.inser_classcontrol(req, res)
    res.send({
      status: 200,
      typo: 'succes',
      messege: result
  ...
  async list_classcontrol (req, res) {
    const result = await objclasscontroles.list_classcontrol(req, res)
    res.json(result)
  }
}
