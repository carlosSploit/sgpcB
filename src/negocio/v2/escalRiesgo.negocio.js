...
module.exports = class ngescalRiesg {
  async inser_escalRiesgo (req, res) {
    const result = await objescalRiesgo.inser_escalRiesgo(req, res)
    res.send({
      status: 200,
      typo: 'succes',
      messege: result
  ...
  async list_escalRiesgo (req, res) {
    const result = await objescalRiesgo.list_escalRiesgo(req, res)
    res.json(result)
  }
}
