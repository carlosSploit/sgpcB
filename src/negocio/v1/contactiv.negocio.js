...
const Bdcontactiva = require('../model/bd_contactiv')
const objcontactiva = new Bdcontactiva()
...
module.exports = class ngsesion {
  async inser_contactiva (req, res) {
    ...
    const result = await objcontactiva.inser_contactiva(req, res)
    res.send({
      status: 200,
      typo: 'succes',
      messege: result
  ...
  async actualise_contactiva (req, res) {
    ...
    // let results = await conexibd.single_query(req, res,'CALL `insert_admin`(?,?,?,?,?,?);',[req.body.name, req.body.telf, req.body.correo, req.body.pass, req.body.tipoadm, req.body.photo])
    // return (Array.isArray(results))?results:[];
    const result = await objcontactiva.actualise_contactiva(req, res)
    res.send({
      status: 200,
      typo: 'succes',
      messege: result
  ...
  async list_contactiva (req, res) {
    ...
    // return (Array.isArray(results))?results:[];
    const result = await objcontactiva.list_contactiva(req, res)
    res.json(result)
  }
  ...
  async delect_contactiva (req, res) {
    ...
    const result = await objcontactiva.delect_contactiva(req, res)
    res.send({
      status: 200,
      typo: 'succes',
      messege: result
