...
// ######################### DbclientAnali ###################################
module.exports = class ETLtipoAmenas {
  async Load () {
  }

  async Extrac () {
    const LoadFilte = fs.readFileSync(require.resolve('./data/tiposSalvaguarda.csv'), { encoding: 'utf8' })
  ...
  // INSERT INTO `tiposalvaguarda`(`id_tipoSalvaguarda`, `nombreTipoSalg`, `estade`) VALUES
  ...
  async Trasform (mintCapDatos = 2, limitInsert = 0) {
    ...
    const dataConsult = LoadDataCVS.map((item, index) => {
      ...
      const FormatoInserDatos = [dataTrasnforColumn[0], dataTrasnforColumn[1], '1']
