...
// ######################### DbclientAnali ###################################
module.exports = class ETLafectaTip {
  async Load () {
  }

  async Extrac () {
    const LoadFilte = fs.readFileSync(require.resolve('./data/afectaTip.csv'), { encoding: 'utf8' })
  ...
  // INSERT INTO `afectatip`(`id_adectTip`, `id_amenazas`, `id_libreryAmen`, `id_tipoActivo`, `estade`) VALUES
  ...
  async Trasform (mintCapDatos = 2, limitInsert = 0) {
    ...
    const dataConsult = LoadDataCVS.map((item, index) => {
      ...
      const FormatoInserDatos = [dataTrasnforColumn[0], dataTrasnforColumn[1], dataTrasnforColumn[2], dataTrasnforColumn[3], '1']
