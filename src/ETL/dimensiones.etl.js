...
module.exports = class DbDependeciActivArbol {
  ...
  async Extrac () {
    const LoadFilte = fs.readFileSync(require.resolve('./data/dimensiones.csv'), { encoding: 'utf8' })
  ...
  // INSERT INTO `dimensionanalisis`(`id_dimension`, `nombreDimens`, `abreb`, `descripc`, `preguAnalis`, `estade`)
  ...
  async Trasform (mintCapDatos = 2, limitInsert = 0) {
    ...
    const dataConsult = LoadDataCVS.map((item, index) => {
      ...
      const FormatoInserDatos = [dataTrasnforColumn[0], dataTrasnforColumn[2], dataTrasnforColumn[1], dataTrasnforColumn[3], dataTrasnforColumn[4], '1']
