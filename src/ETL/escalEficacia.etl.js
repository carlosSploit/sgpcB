...
module.exports = class ETescalDegrad {
  ...
  async Extrac () {
    const LoadFilte = fs.readFileSync(require.resolve('./data/escalEficacia.csv'), { encoding: 'utf8' })
  ...
  // INSERT INTO `escalaefica`(`id_escalEfic`, `nombreEscalEfic`, `codigEscalEfic`, `valorEscalEfic`, `rangValEscalEfic`, `estade`) VALUES
  ...
  async Trasform (mintCapDatos = 2, limitInsert = 0) {
    ...
    const dataConsult = LoadDataCVS.map((item, index) => {
      ...
      const FormatoInserDatos = [dataTrasnforColumn[0], dataTrasnforColumn[2], dataTrasnforColumn[1], LoadDataCVS.length - index, dataTrasnforColumn[3], '1']
