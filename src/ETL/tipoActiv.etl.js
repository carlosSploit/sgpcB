...
module.exports = class DbtipoActiv {
  ...
  async formatMYSQL (consulte = 'INSERT INTO [nameTable] VALUES', limitInsert = 40, mintCapDatos = 2) {
  ...
  async Trasform (mintCapDatos = 2, limitInsert = 40) {
    ...
    const dataConsult = LoadDataCVS.map((item, index) => {
      ...
      const FormatoInserDatos = [dataTrasnforColumn[0], dataTrasnforColumn[3], dataTrasnforColumn[1], dataTrasnforColumn[2], (parseInt(dataTrasnforColumn[4]) === 0) ? '0' : '1', dataTrasnforColumn[4], '1']
      return FormatoInserDatos
    // eslint-disable-next-line eqeqeq
    }).filter(item => item.length > mintCapDatos).filter((item, index) => { return index < limitInsert })
    return dataConsult
  }
